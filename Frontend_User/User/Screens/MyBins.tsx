  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Your Bins</Text>
      </View>
      <ScrollView style={styles.scrollView}>
        {bins.length > 0 ? (
          bins.map((item,index) => (
            <TouchableOpacity key={item.id} style={styles.binItem}>
              <View style={[styles.binColorIndicator, { backgroundColor: item.customBinColor }]} />
              <View style={styles.binContent}>
                <View style={styles.binImageContainer}>
                  <Image source={binImages[ index % binImages.length]}style={styles.binImage} />
                    <Text style={styles.binTypeText}>{item.binType}</Text>
                  
                </View>
                <View style={styles.binInfo}>
                  <Text style={styles.binTitle}>{item.binType} Bin : {item.binID}</Text>
                  <Text style={styles.wasteTypeText}>Waste Type: {item.type.wasteType}</Text>
                  <View style={styles.wasteLevelContainer}>
                    
                    <Text style={styles.wasteLevelText}>Waste Level:</Text>
                    <Text style={[styles.wasteLevelValue, { color: item.wasteLevel > 70 ? '#FF4136' : '#2ecc71' }]}>
                      ...
                      <View 
                        style={[
                          ...
                          { 
                            ...
                            backgroundColor: item.wasteLevel > 70 ? '#FF4136' : '#2ecc71'
                          }
                        ]}
                      />
                    </View>
                    {item.wasteLevel > 70 && (
                      <AlertTriangle size={16} color="#FF4136" style={styles.alertIcon} />
                    )}
                  </View>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  wasteTypeText: { // New style for waste type text
    fontSize: 14,
    color: '#666666',
    marginTop: 4,
  },
