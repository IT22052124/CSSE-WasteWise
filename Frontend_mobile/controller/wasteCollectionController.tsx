...
export const getBinTypeDetailsByID = async (binTypeID) => {
  try {

    console.log("here")
    // Step 2: Fetch the document from Firestore
    const binTypeDoc = await getDoc(binTypeID);

    console.log(binTypeDoc.data());

    // Step 3: Check if the document exists
    if (!binTypeDoc.exists()) {
      console.error("No such binType document!");
      return null; // or handle it as per your requirements
    }

    // Step 4: Extract the necessary fields (chargingPerKg, incentivesPerKg)
    const { chargingPerKg, incentivesPerKg } = binTypeDoc.data();

    // Step 5: Return the values or any other data you want
    return {
      chargingPerKg,
      incentivesPerKg,
    };
  } catch (error) {
    console.error("Error retrieving binType details:", error);
    throw new Error("Failed to retrieve binType details");
  }
};
